...
public:
  using type =
      std::conditional_t<sizeof(current_t) >= sizeof(max_t), current_t, max_t>;
};
...
template <typename T, typename D> struct Has<T, D, false> {
  constexpr static bool value =
      std::is_same_v<T, front<D>> || Has<T, pop_front<D>>::value;
};
...
int main() {
  ...
  static_assert(has_v<int, Deque<double, char>> == false);
}
